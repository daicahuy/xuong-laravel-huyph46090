...
<body>
    <div class="container">
        <h1 class="text-center mb-5">Edit Employee {{ $employee->first_name }} {{ $employee->last_name }}</h1>
        @if (session()->has('success') && session('success'))
            <div class="alert alert-success">Sua thanh cong</div>
        @endif
        @if (session()->has('success') && !session('success'))
            <div class="alert alert-danger">Sua that bai</div>
        @endif
        @if ($errors->any())
        ...
        <form action="{{ route('employees.update', [$employee]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row mb-2">
                <div class="col-5">
                    ...
                    <input type="text" class="form-control" value="{{ $employee->department_id }}" name="department_id">
                </div>
                <div class="col-5">
                    <label for="" class="form-label">Manager id</label>
                    <input type="text" class="form-control" value="{{ $employee->manager_id }}" name="manager_id">
                </div>
                <div class="col-2">
                    <label for="" class="form-label d-block">Active</label>
                    <input type="checkbox" class="form-check-input" @checked($employee->is_active) name="is_active"
                        value="1">
                </div>
            </div>
            <div class="row mb-2">
